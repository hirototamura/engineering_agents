#!/usr/bin/env bash
# Space Station OS — interactive Docker shell (Mac + Colima).
#
# Mounts engineering_agents src/results and SSOS helper scripts into /root/.
# For headless sim inside the container: bash /root/ssos-eclss-headless.sh
#
# For host `ea run` (detached, no shell): use ssos-run-detached.sh instead.
set -euo pipefail
...
echo "    Stop sim:     Ctrl+C (during ros2 launch)"
echo "    Force stop:   docker stop ${CONTAINER_NAME}"
