#!/usr/bin/env bash
# Phase 1b: run SSOS ECLSS ARS+OGS bridge smoke inside the SSOS Docker container.
#
# Prerequisite: ECLSS stack running (bash /root/ssos-eclss-headless.sh)
#
#   ./scripts/run_ssos_eclss_1b_smoke.sh
#   ./scripts/run_ssos_eclss_1b_smoke.sh --json-out /tmp/eclss_1b_smoke.json
#
set -euo pipefail
...
run_smoke() {
  ...
  shift
  cd "$workdir"
  PYTHONPATH="$workdir/src${PYTHONPATH:+:$PYTHONPATH}" python3 -m scripts.ssos_eclss_1b_smoke "$@"
}
...
echo "==> Running Phase 1b smoke in container '$CONTAINER'"
if [ -t 0 ]; then
  ...
  cd '$CONTAINER_REPO'
  PYTHONPATH='$CONTAINER_REPO/src:'\"\${PYTHONPATH:-}\" python3 -m scripts.ssos_eclss_1b_smoke${quoted_args}
"
else
