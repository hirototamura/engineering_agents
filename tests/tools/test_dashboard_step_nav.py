...
def test_should_show_step_slider_requires_strictly_increasing_bounds():
    assert _should_show_step_slider(1, 1) is False
    assert _should_show_step_slider(0, 0) is False
    assert _should_show_step_slider(1, 2) is True
    assert _should_show_step_slider(0, 7) is True
...
def test_step_nav_controls_zero_based_single_step_run():
    app = AppTest.from_string(
        """
from tools.dashboard.app import _step_nav_controls
import streamlit as st
...
"""
    ...
    assert list(app.slider) == []
    captions = [c.value for c in app.caption]
    assert any("Replay step: 0" in value for value in captions)
