...
うまくいった設計は棄却されたのではない。**忘れられた。**各周は自分のランから状態を組み立て直す——それが周を監査可能にしている当のものであり、連鎖に記憶が無かった理由でもある。連鎖の記憶が持つのは4つだけ。全員生存した最良設計、前周に**実際に設置された**寸法、各サブシステムの理論下限、この連鎖が既に踏んだ失敗を最大5件。4 KB を超えたときは文章を切らず、最も価値の低い項目を丸ごと落とす。
...
50周の連鎖を3本、同じ世界・同じ乗員で。その間に変更は2つ。以下の数値はすべて [`docs/data/`](docs/data) にコミットした周回ごとの指標から読んでいる。

| | 段階① 初期 | 段階② ＋連鎖の記憶 | 段階③ ＋採点表の基準変更 |
| --- | ---: | ---: | ---: |
| **最終生存者** | **34/50** | **50/50** | **50/50** |
| 0/50 の周 | 12 | 1 | 1 |
| 3項目そろった提案 | 38/50 | 50/50 | 50/50 |
| ユニーク設計数 | 39 | 11 | 17 |
| 最高 / 平均スコア | 66.18 / 61.71 | 66.36 / 65.94 | 84.23 / 83.34 |

*段階③のスコアは①②と比較できない（採点関数そのものを変えたため）。比較できるのは生存者数・巻き戻り・提案の完全性・ユニーク設計数。*

![3段階の生存者数とスコア](docs/images/results/ssos_phase1_phase2_phase3_survival_score_trend.svg)
...
- **P1** — 連鎖の記憶を「見せる」から「適用する」へ。適用済み提案をマージし、名指しされなかったフィールドが設置済みの値を保つようにする。
- **P1** — 1軸より多く探索する。ARS/OGS は理論下限に達して以降動いていない。
