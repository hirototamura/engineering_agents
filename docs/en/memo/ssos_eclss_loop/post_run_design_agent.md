...
- `set_parameter` (`thresholds.*`) is not on the apply allow-list and is not auto-applied in the chain. `ea run` defaults to `--approve-provisional` (INFO: the sim auto-approves LLM designs so the loop can run without a human). `--no-approve-provisional` restores the supervisor gate
