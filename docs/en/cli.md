...
| `--llm-model` | Override LLM model; on ssos writes both `agents.actor.llm.model` and `agents.design.llm.model`. Distinct URLs/models: `--set agents.actor.llm.base_url=` / `--set agents.design.llm.base_url=` (same for `.model`). Avoid `VLLM_BASE_URL` / `VLLM_MODEL` when you need the split |
