...
| `--approve-provisional` / `--no-approve-provisional` | Adopt `provisional_final` LLM designs. Omit to use `scenario.yaml` `iteration.approve_provisional` (`ssos_eclss_loop`). `--no-approve-provisional` restores the supervisor gate |
