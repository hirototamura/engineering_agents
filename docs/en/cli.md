...
| `--approve-provisional` / `--no-approve-provisional` | Adopt `provisional_final` LLM designs. **Default on** for `ssos_eclss_loop` so the sim can close the loop without a human (prints an INFO note). `--no-approve-provisional` restores the supervisor gate |
