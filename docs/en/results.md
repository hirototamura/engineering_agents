# Experiment record — four fifty-round chains

Four fifty-round design→verify chains were run against the same world, the same crew of fifty, and the same non-survivable baseline. Three things changed: first the chain was given a memory, then the scorecard's cost and mass axes were re-anchored, then three auditors were placed beside the designer. Nothing else — same simulator, same actors.
...
| Rounds at 50/50 | 34 | 49 | 48 | 48 |
| Rounds at 0/50 | 12 | 1 | 1 | 1 |
| Complete design proposals | 38/50 | 50/50 | 50/50 | 50/50 |
| Catastrophic ARS/OGS reset | 12 | first round only | first round only | none |
...
!!! warning "Phase 3 and 4 scores are not comparable to phases 1–2"
    Phase 3 changed the scoring function. Its 84-point results are largely the cost/mass axes paying out differently, **not** a design that got 18 points better. Phase 4 uses the same scorecard as phase 3, so those two *are* comparable. The behavioural facts that *are* comparable across all four: survivors, reset count, proposal completeness, unique designs.

![Survivors and score across all four phases](../images/results/ssos_phase1_phase2_phase3_survival_score_trend.svg)
...
## Phase 4 — the auditors stopped the danger, and the search

The only change is on the design side. One unlensed designer proposes; three independent auditors (`rederive_numbers` / `avoid_local_optima` / `design_validity`) merge by item-veto. The scorecard is the phase-3 sheet, so the scores compare.

Round 2 raised OGS to 48.0 and left ARS at the shipped 4.5 — 19/50. Round 3 reached 25.0 / 48.0 / 10.0 and 50/50. Then a monotonic shrink, and by round 17 the machine is `ARS=20.8, OGS=42.0, WRS=1.65`. That is also the final answer (round 18). The remaining thirty-four rounds flew the same airframe.

The designer tried to cut WRS to 1.58. Auditors 2 and 3 vetoed it — 0.0175 above the calculated 1.5625 floor. Phase 3's round 34 (WRS=1.25, 45/50) never happened.

The cost is in the same table. Unique designs fell from 17 to 9. Best score fell from 84.23 to 84.03. The chain never visited phase 3's promising WRS 1.8–2.2 band, nor the 1.25 failure boundary. The auditors kept the machine from breaking. They also kept it from searching. Both are true.

![Phase 4 score composition](../images/results/phase4_score_components_grouped.svg)

![Phase 4, all seven axes](../images/results/phase4_score_components_stacked_split.svg)

Best = final (rounds 17 / 50): A 20.00 · B 10.00 · C 9.98 · D 6.09 · E 14.83 · F 14.78 · G 8.35 · **84.03 total**. 4077 kg / 603 M$ — physically the same neighbourhood as phase 3's last round.

---

## What the design variables actually did

![ARS / OGS / WRS across all four phases](../images/results/ssos_phase1_phase2_phase3_parameter_trends.svg)
...
Promising range: WRS 1.8–2.2. Failure boundary: WRS ≈ 1.25. Phase 4's modal design is `20.8 / 42.0 / 1.65` for 34 rounds — the auditors called that sufficient, and nothing moved after.
...
2. **The search is still WRS-only, and phase 4 stopped even that.** ARS and OGS have not been perturbed since they hit the floor. Phase 3 at least swept WRS. Phase 4's auditors vetoed the cut to 1.58, and from round 17 the same design flew thirty-four times.
...
6. **One model, one seed, four chains.** These are four runs, not a statistical study. They are enough to show a mechanism working and a mechanism failing; they are not enough to put an error bar on a score. The chain with `floor_probe.py` has still not been run.
...
All four chains are archived whole — [`experiments/runs/`](https://github.com/hirototamura/engineering_agents/tree/main/experiments/runs), 11 MB compressed each — together
