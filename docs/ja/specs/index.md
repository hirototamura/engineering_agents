...
**実装**: `<chain_dir>/compact_chain_memory.json` 一枚（4 KB 上限）。全員生存した最良設計、直前に実際に設置された寸法、各サブシステムの理論下限、この連鎖が既に踏んだ失敗パターンだけを持つ。読み手が有限コンテキストの言語モデルなので、サイズは実装詳細ではなく設計制約である。
...
**未解決として残っている点**: chain memory は**見せるだけで適用しない**。部分提案が省いたフィールドが落ちる挙動そのものは直っていない（累積設計状態として merge する修正が別途要る）。実走行では「見せる」だけで巻き戻りは初回以外消えた（[実験記録](../results.md)）。
