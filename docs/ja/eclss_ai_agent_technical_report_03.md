...
| 13 | 創発の可視化。予想外の変数操作、設置クラスと reasoning 意図の対応、**言ったことと置いた機体の一致率**（① 2% → ③ 43% → ④ 6%）、シングル設計者 vs 監査パネルの思想の切れ目 |
...
4 本の LLM 連鎖を同じ ρ 座標に載せると、プラント側の臨界とエージェント側の固定点が重なる。

![4 連鎖のカバレッジ軌跡](../images/results/report03_rho_tracks.svg)

*図 11-1. 段階①–④の ρ 軌跡。破線は理論下限（ρ = 1）。ガスは段階②以降ほぼ下限に張り付き、動くのは水だけである。段階①だけがガスを出荷値（ρ ≪ 1）へ何度も落とす。*
...
*図 13-1. 段階①–④、設置値の差分から分類した各 49 遷移。段階①は `gas_oversize` 19 + `baseline_revert` 12。段階②③は `wrs_trim`/`wrs_restore` が大半。段階④は `freeze` 41。*
...
*図 13-2. 設計者 reasoning から分類した意図。段階①でも「WRS を削る」が 31 周ある。段階④では 45 周が「WRS を削る」。*
...
*図 13-3. 意図クラスと設置差分クラスの一致率。記憶+採点で 43% まで揃い、監査を入れると 6% に落ちる。*
...
*図 13-4. 左: ユニーク設計数。中: 設置値が前周と同一の周。右: 全員生存設計の WRS 幅。④の freeze 34 と unique 9 は ver.02 の集計。①–③は CSV。*

![段階④ 監査パネルの票](../images/results/report03_audit_votes.svg)

*図 13-5. 監査 3 レンズの票。局所最適の回避は 44 回 reject / 5 回 approve。数値の再導出は 34 回 approve。拒否の本体は「探索を止めよ」であり、「数が合わない」ではない。*
...
4. **言ったことと置いた機体は、監査で再び割れる。** 本文↔設置の一致は① 2% → ③ 43% → ④ 6%。段階④の設計者は 45 周「WRS を削る」と書いた。置かれた機体は 41 周凍結である。思想の切れ目は設計者の文ではなく、局所最適回避レンズの reject 44 票にある。
5. **採点の改善と物理の改善は今も混同しやすい。** 段階③の +18 点はレンジ補正である。ver.02 が書いたこの注意は、アンサンブル（予算内生存 0 / 121）と合わせて読むとより強い。ミッションを満たす設計は、宣言された予算の外にある。

再現: 段階①–④の表は `experiments/analysis/analyze_ssos_iter.py` が CSV から再生成する。プラント側の ρ・ゲイン・アーキタイプは `python3 -m tools.analysis report`。創発の集計は `docs/data/report03_emergence.json`。図 11-1 / 12-1 / 13-1–13-5 は `experiments/analysis/make_report03_emergence_figures.py` が同じ CSV から描く。
