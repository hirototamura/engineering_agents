...
- `set_parameter`（`thresholds.*`）は連鎖では自動適用しない。`iteration.approve_provisional` の既定は false。`--approve-provisional` で opt-in（INFO）。labeled/mock の `provisional_final` を人間なしで載せるときだけ使う。
