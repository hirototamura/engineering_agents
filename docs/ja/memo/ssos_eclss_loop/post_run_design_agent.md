...
- `set_parameter`（`thresholds.*`）は許可リストに無く、連鎖でも自動適用しない。`iteration.approve_provisional` の既定は false。`--approve-provisional` で opt-in（INFO）。labeled/mock の `provisional_final` を人間なしで載せるときだけ使う。
