...
- **タイミング指定の注入**は backend ではなくシナリオ層の `subsystem_failures`（`scenario.yaml`）が担当する。任意 step で `set_subsystem_failure` を呼ぶ（`mock` / `plant_sim` / `ros2` 共通）。詳細は [scenario-ssos-eclss-loop.md](../../scenario-ssos-eclss-loop.md#サブシステム故障スケジュールsubsystem_failures)
