...
同じ世界、同じ50人の乗員、同じ「生存不可能なベースライン」に対して、50周の design→verify 連鎖を3本走らせた。3本の間で変えたのは2つだけ。まず連鎖に記憶を与え、次に採点表の費用・質量軸の基準点を移した。それ以外は同一（同じシミュレータ、同じ actor、同じツール）。
...
| --- | ---: | ---: | ---: |
| 周回数 | 50 | 50 | 50 |
| baseline replay 生存者 | 0/50 | 0/50 | 0/50 |
| **final replay 生存者** | **34/50** | **50/50** | **50/50** |
| 50/50 生存の周 | 34 | 49 | 48 |
| 0/50 の周 | 12 | 1 | 1 |
| 3項目そろった提案 | 38/50 | 50/50 | 50/50 |
| ARS/OGS の致命的巻き戻り | 12回 | 初回のみ | 初回のみ |
| ユニーク設計数 | 39 | 11 | 17 |
| 最高スコア | 66.18 | 66.36 | 84.23 |
| 平均スコア | 61.71 | 65.94 | 83.34 |
| 最終スコア | 55.41 | 66.36 | 84.08 |

!!! warning "段階③のスコアは①②と比較できない"
    段階③は採点関数そのものを変えている。84点台は費用・質量軸の支払われ方が変わった結果であって、**設計が18点ぶん良くなったのではない**。3本を通して比較できるのは挙動のほう——生存者数、巻き戻り回数、提案の完全性、ユニーク設計数。
...
変えたのは1つだけ。`compact_chain_memory.json`（4096 バイト上限）。全員生存した最良設計、前周に実際に設置された寸法、各サブシステムの理論下限、この連鎖が既に踏んだ失敗を最大5件。次の周に**見せる**だけで、何も適用しない。
...
段階②と③では ARS が 20.8 kg/day、OGS が 42.0 kg/day に落ち着いて動かない。これは恣意的な値ではなく、`compute_theoretical_capacity` がこの乗員に必要と報告するネームプレートそのものである（指標 CSV の全行で `theory_ars_required_nameplate=20.8`, `theory_ogs_required_nameplate=42.0`。OGS 42.0 は 50 × 0.84 kg/day）。連鎖は物理的な下限を見つけ、その下へ行くことを拒否した。これは正しい挙動であり、chain memory の `theoretical_floor` はそのためにある。
...
有望レンジ WRS 1.8–2.2。失敗境界 WRS ≒ 1.25。
...
2. **探索がいまだ WRS のみ。** ARS/OGS は下限に達して以降動いていない。停滞検知は発火して探索を要求しているが、返ってくる探索は依然として1軸である。
3. **chain memory は見せるだけで適用しない。** 部分提案は今も省いたフィールドを落とす。見せるだけで実測上は足りた（巻き戻り12回→1回）が、適用済み設計のマージは未修正であり、`chain_memory.py` のドックストリング自身がそう書いている。
...
5. **モデル1つ、シード1つ、連鎖3本。** これは3回のランであって統計的な研究ではない。機構が働くこと・働かないことを示すには十分だが、スコアに誤差棒を付けるには足りない。
...
- [実装仕様書](specs/index.md) — この3つの変更がそれぞれ何に対して書かれたか
