...
| `theoretical_floor` | 各サブシステムの物理的下限。トレースから計算 |
...
**コードのドックストリング自身が書いている限界:** chain memory は**見せるだけで、適用はしない**。部分提案は今も省いたフィールドを落とす。見せるだけで実測上は破綻が止まった（50周中の巻き戻りが12回から1回へ）が、根本のマージは未修正であり、ドックストリングにそう書いてある。
