...
components = [
    ...
    ("B TCL", "B", "#40916c", lambda r: read_float(r, "axis_tcl")),
    ("C Environment", "C", "#277da1", lambda r: read_float(r, "axis_environment_trajectory")),
    ("D Recovery", "D", "#f9c74f", lambda r: read_float(r, "axis_resource_recovery")),
    ("E Cost", "E", "#f3722c", lambda r: read_float(r, "axis_cost")),
    ("F Mass", "F", "#b56576", lambda r: read_float(r, "axis_mass")),
    ("G Ops/Physics", "G", "#9d4edd", lambda r: read_float(r, "axis_actor_decision") + read_float(r, "axis_physical_response")),
]

with (out / f"{args.prefix}_score_components_split.csv").open("w", encoding="utf-8", newline="") as fh:
...
for label, short, color, _ in components:
    ...
    elems.append(f'<text x="{legend_x+22}" y="{legend_y}" font-size="13" font-family="Arial">{short}: {label[2:]}</text>')
    legend_y += 26

elems.append(f'<text x="{legend_x}" y="{legend_y+12}" font-size="12" font-family="Arial" fill="#555">Cost and mass are separate</text>')
elems.append(f'<text x="{legend_x}" y="{legend_y+32}" font-size="12" font-family="Arial" fill="#555">G = actor decision + physical response</text>')
elems.append("</svg>")

(out / f"{args.prefix}_score_components_stacked_split.svg").write_text("\n".join(elems), encoding="utf-8")
